...
def _scientific_validation_rule(game_size: int) -> dict[str, Any]:
    ...
    if validation_threshold is None:
        validation_threshold = 13 if resolved_game_size >= 18 else 12 if resolved_game_size >= 17 else 11
    target_band = target_band_by_game_size.get(resolved_game_size, f"{validation_threshold}_to_15")
    return {
...
class LotofacilScientificCore:
    ...
    def build_post_reconciliation_scientific_memory(
        ...
        *,
        ...
        game_size = len(game_numbers[0]) if game_numbers else len(_normalize_numbers(game_rows[0].get("numbers", []))) if game_rows else 15
        ...
        return {
            ...
            "scientific_status": "APROVADO" if validation_count_plus > 0 else "REPROVADO",
            "main_reason": (
                ...
                f"minimo_premiavel_{validation_threshold}" if validation_count_plus > 0 else
                "near_miss_local"
            ),
            "best_hit": best_hits,
            "average_hits": average_hits,
            ...
            "approved_for_use": int(validation_count_plus > 0 and concentration_risk <= 0.5),
            "notes": (
        ...
        batch_game_size = (
            ...
            if best_game_numbers
                ...
                if best_generation_games
                    ...
                    len(_normalize_numbers((generation_rows[0].get("games", [{}]) or [{}])[0].get("numbers", [])))
    ...
    def build_batch_reconciliation_scientific_memory(
        ...
        *,
        ...
        total_games_checked = sum(int(item.get("total_games", 0) or 0) for item in analyzed_generations)
        global_best_hits = max((item.get("best_hits", 0) or 0 for item in analyzed_generations), default=0)
        global_count_10 = sum(item.get("count_10", 0) or 0 for item in analyzed_generations)
        global_count_11_plus = sum(item.get("count_11_plus", 0) or 0 for item in analyzed_generations)
        global_count_12_plus = sum(item.get("count_12_plus", 0) or 0 for item in analyzed_generations)
        global_count_13_plus = sum(item.get("count_13_plus", 0) or 0 for item in analyzed_generations)
        global_count_14_plus = sum(item.get("count_14_plus", 0) or 0 for item in analyzed_generations)
        global_count_15 = sum(item.get("count_15", 0) or 0 for item in analyzed_generations)
        batch_game_size = (
            len(_normalize_numbers((generation_rows[0].get("games", [{}]) or [{}])[0].get("numbers", [])))
            if generation_rows and generation_rows[0].get("games")
            else 15
        ...
        for generation in analyzed_generations:
            ...
            generation_games = [dict(game or {}) for game in generation.get("games", []) or []]
            generation_results = [dict(result or {}) for result in generation.get("results", []) or []]
            ...
            for index, result in enumerate(generation_results):
                ...
                source_game = generation_games[index] if index < len(generation_games) else {}
                numbers = _normalize_numbers(result.get("numbers", source_game.get("numbers", [])))
                source_context = dict(source_game.get("context_json") or {})
                detail = {
                    "generation_event_id": generation_id,
                    "batch_id": str(generation.get("batch_id", resolved_batch_id) or resolved_batch_id),
                    "game_index": int(result.get("game_index", source_game.get("game_index", index + 1)) or index + 1),
                    "game_signature": str(
                    ...
                    "missing_draw_numbers": list(result.get("missing_draw_numbers", []) or []),
                    "extra_numbers": list(result.get("extra_numbers", []) or []),
                    "score_original": float(result.get("score_original", source_game.get("score", 0.0)) or 0.0),
                    "perfil": str(
                        result.get("perfil")
                    ...
                    "contest_number": int(generation.get("contest_number", contest_number or 0) or 0),
                    "source_memory_id": _safe_int(
                        source_context.get("based_on_memory_id", source_context.get("source_memory_id", source_context.get("memory_id"))),
                        default=None,
                    ),
                    "based_on_memory_kind": str(
                        source_context.get("based_on_memory_kind", source_context.get("memory_kind", "")) or ""
                    ),
                    "based_on_batch_id": str(
                        source_context.get("based_on_batch_id", source_context.get("batch_id", "")) or ""
                    ),
                    "hit_classification": (
                        f"EXACT_{hits}"
                        if 11 <= hits <= 15
            ...
            generation_details.append(
                {
                    "generation_event_id": generation_id,
                    "batch_id": str(generation.get("batch_id", resolved_batch_id) or resolved_batch_id),
                    "total_games": int(generation.get("total_games", len(generation_results)) or len(generation_results)),
                    ...
                    "scientific_score": float(generation.get("scientific_score", 0.0) or 0.0),
                    "contest_number": int(generation.get("contest_number", contest_number or 0) or 0),
        ...
        batch_generation_range = {
            ...
            "target_band": str(validation_rule["target_band"]),
            "validation_zone_label": str(validation_rule["validation_zone_label"]),
            "best_generations": [best_generation_event_id] + [item for item in secondary_generation_event_ids if item > 0],
        ...
        best_generation_details = [
            ...
            if int(detail.get("generation_event_id", 0) or 0) in ([best_generation_event_id] + [item for item in secondary_generation_event_ids if item > 0])
        ]
        ...
        payload.update(
            {
                ...
                "based_on_post_reconciliation_memory_id": base_memory.get("based_on_post_reconciliation_memory_id"),
                "best_generation_event_id": best_generation_event_id,
                "best_generations": [best_generation_event_id] + [item for item in secondary_generation_event_ids if item > 0],
                "best_generation_count_10": int(best_generation.get("count_10", 0) or 0),
                "secondary_generation_event_ids": secondary_generation_event_ids,
    ...
    def discover_scientific_generation_policy(
        ...
        *,
        ...
        latest_batch_memory = next(
            (row for row in memory_rows if str(row.get("memory_kind", "") or "").strip() == "scientific_batch_reconciliation"),
            {},
        )
        ...
        with get_session(self.db_path) as session:
            rows = (
                session.query(ScientificInstitutionalMemory)
                .filter(ScientificInstitutionalMemory.memory_kind == "scientific_batch_reconciliation")
                .order_by(
                    ScientificInstitutionalMemory.created_at.desc(),
                    ScientificInstitutionalMemory.id.desc(),
                )
        ...
        prioritized_memory_kind = str(prioritized_memory_row.get("memory_kind", "") or "").strip()
        prioritized_memory_id = int(prioritized_memory_row.get("id", 0) or 0)
        prioritized_batch_id = str(prioritized_memory_row.get("batch_id", "") or "").strip()
        prioritized_generation_range = dict(prioritized_memory_row.get("generation_range", {}) or {})
        prioritized_policy_after = dict(prioritized_memory_row.get("policy_after", {}) or {})
        prioritized_policy_before = dict(prioritized_memory_row.get("policy_before", {}) or {})
        prioritized_policy = dict(prioritized_policy_after or prioritized_policy_before or {})
        if prioritized_policy:
            prioritized_policy = {
                **base_policy,
                **{key: value for key, value in prioritized_policy.items() if key in base_policy},
                **{key: value for key, value in prioritized_policy.items() if key not in base_policy},
            }
        if prioritized_memory_kind == "scientific_batch_reconciliation":
            prioritized_policy.setdefault("policy_adjustment_reason", "recalibrate_from_strong_near_miss_towards_11_plus_and_15")
            prioritized_policy.setdefault("policy_origin", "scientific_batch_reconciliation_memory")
        elif prioritized_memory_kind == "scientific_strong_near_miss":
            prioritized_policy.setdefault("policy_adjustment_reason", "recalibrate_from_strong_near_miss_towards_11_plus_and_15")
            prioritized_policy.setdefault("policy_origin", "scientific_strong_near_miss_memory")
        elif prioritized_memory_kind == "scientific_reconciliation":
            prioritized_policy.setdefault("policy_adjustment_reason", "recalibrate_from_near_miss_towards_15")
            prioritized_policy.setdefault("policy_origin", "scientific_reconciliation_memory")
        elif prioritized_memory_kind == "scientific_calibration":
            prioritized_policy.setdefault("policy_origin", "scientific_calibration_memory")
        memory_policy = dict(prioritized_policy) if prioritized_policy else {}
        latest_persisted_batch_memory = batch_memory_rows[0] if batch_memory_rows else latest_batch_memory
        previous_persisted_batch_memory = batch_memory_rows[1] if len(batch_memory_rows) > 1 else {}
        latest_batch_generation_range = dict(latest_persisted_batch_memory.get("generation_range", {}) or {})
        previous_batch_generation_range = dict(previous_persisted_batch_memory.get("generation_range", {}) or {})
        batch_memory_current_best_hits = int(latest_batch_generation_range.get("global_best_hits", latest_persisted_batch_memory.get("best_hit", 0)) or 0)
        batch_memory_current_count_10 = int(latest_batch_generation_range.get("global_count_10", 0) or 0)
        batch_memory_current_count_11_plus = int(latest_batch_generation_range.get("global_count_11_plus", latest_persisted_batch_memory.get("count_11_plus", 0)) or 0)
        batch_memory_previous_best_hits = int(previous_batch_generation_range.get("global_best_hits", previous_persisted_batch_memory.get("best_hit", 0)) or 0)
        batch_memory_previous_count_10 = int(previous_batch_generation_range.get("global_count_10", 0) or 0)
        batch_memory_previous_count_11_plus = int(previous_batch_generation_range.get("global_count_11_plus", previous_persisted_batch_memory.get("count_11_plus", 0)) or 0)
        batch_memory_has_evolution = batch_memory_current_count_11_plus > 0
        hybrid_auxiliary_mode = prioritized_memory_kind == "scientific_batch_reconciliation" and not batch_memory_has_evolution
        ...
        def _load_batch_games_for_cross_validation(batch_id: str) -> list[dict[str, Any]]:
            resolved_batch_id = str(batch_id or "").strip()
            ...
            with get_session(self.db_path) as session:
                rows = (
                    ...
                    .filter(GenerationEvent.context_json["batch_id"].as_string() == resolved_batch_id)
                ...
                games.append(
                    {
                        ...
                        "perfil": str(context_json.get("perfil", getattr(row, "profile_type", "") or "") or getattr(row, "profile_type", "") or ""),
                        "profile_type": str(getattr(row, "profile_type", "") or context_json.get("profile_type", "") or ""),
                    }
                )
            return games
        ...
        def _build_cross_validation_summary(batch_id: str) -> dict[str, Any]:
            ...
            if not games:
                with get_session(self.db_path) as session:
                    memory_row = (
                        session.query(ScientificInstitutionalMemory)
                        .filter(ScientificInstitutionalMemory.batch_id == resolved_batch_id)
                        .order_by(
                            ScientificInstitutionalMemory.created_at.desc(),
                            ScientificInstitutionalMemory.id.desc(),
                        )
                ...
                if memory_row:
                    recovered_summary = dict(getattr(memory_row, "cross_validation_summary", {}) or {})
                    recovered_windows = dict(recovered_summary.get("windows", recovered_summary.get("cross_validation_windows", {})) or {})
                    if recovered_windows:
                        total_count_11_plus = sum(int(window.get("total_count_11_plus", 0) or 0) for window in recovered_windows.values())
                        contests_with_11_plus = sum(int(window.get("contests_with_11_plus", 0) or 0) for window in recovered_windows.values())
                        support_level = str(recovered_summary.get("support_level", "") or "").strip() or (
                            "dominant_conditional"
                            if any(
                                float(window.get("average_best_hits", 0.0) or 0.0) >= 11.0
                                and int(window.get("max_best_hits", 0) or 0) >= 13
                                for window in recovered_windows.values()
                            )
                            else "strong_support"
                            if any(float(window.get("average_best_hits", 0.0) or 0.0) >= 11.0 for window in recovered_windows.values())
                            else "none"
                        )
                        cross_validation_reason = str(recovered_summary.get("cross_validation_reason", "") or "").strip() or (
            ...
            def _window_summary(window_size: int) -> dict[str, Any]:
                ...
                for contest in window_contests:
                    contest_numbers = set(_normalize_numbers(contest.get("numbers", [])))
                    per_game: list[dict[str, Any]] = []
                    for game in games:
                        matched_numbers = sorted(contest_numbers.intersection(game["numbers"]))
                        missing_draw_numbers = sorted(contest_numbers.difference(game["numbers"]))
                        extra_numbers = sorted(set(game["numbers"]).difference(contest_numbers))
                    ...
                    if not per_game:
                        continue
                    best_game = max(per_game, key=lambda item: (int(item["hits"]), float(item["score_original"]), -int(item["game_index"])))
                    best_hits = int(best_game["hits"])
                    count_10 = sum(1 for item in per_game if int(item["hits"]) == 10)
                    count_11_plus = sum(1 for item in per_game if int(item["hits"]) >= 11)
                    count_12_plus = sum(1 for item in per_game if int(item["hits"]) >= 12)
                    count_13_plus = sum(1 for item in per_game if int(item["hits"]) >= 13)
                    count_14_plus = sum(1 for item in per_game if int(item["hits"]) >= 14)
                    ...
                    if count_11_plus > 0:
                        contest_numbers_with_11_plus.append(int(contest["contest_number"]))
                    if count_12_plus > 0:
                        contest_numbers_with_12_plus.append(int(contest["contest_number"]))
                    if count_13_plus > 0:
                        contest_numbers_with_13_plus.append(int(contest["contest_number"]))
                    if count_14_plus > 0:
                        contest_numbers_with_14_plus.append(int(contest["contest_number"]))
                    if count_15 > 0:
                    ...
                    contest_details.append(
                        {
                            ...
                            "count_14_plus": count_14_plus,
                            "count_15": count_15,
                            "best_game_signature": str(best_game.get("game_signature", "") or ""),
                            "matched_numbers": list(best_game.get("matched_numbers", []) or []),
                            "missing_draw_numbers": list(best_game.get("missing_draw_numbers", []) or []),
                            "extra_numbers": list(best_game.get("extra_numbers", []) or []),
                            "best_games": sorted(per_game, key=lambda item: (-int(item["hits"]), -float(item["score_original"]), int(item["game_index"])))[:3],
                        }
                    )
                average_best_hits = round(_mean_or_zero([float(value) for value in best_hits_values]), 4) if best_hits_values else 0.0
                ...
                return {
                    ...
                    "contest_numbers": [int(item["contest_number"]) for item in window_contests],
            ...
            total_count_11_plus = sum(int(window.get("total_count_11_plus", 0) or 0) for window in windows.values())
            contests_with_11_plus = sum(int(window.get("contests_with_11_plus", 0) or 0) for window in windows.values())
        ...
        batch_cross_validation_summary = _build_cross_validation_summary(prioritized_batch_id) if prioritized_memory_kind == "scientific_batch_reconciliation" else {}
        batch_cross_validation_support_level = str(batch_cross_validation_summary.get("support_level", "") or "").strip()
        batch_cross_validation_reason = str(batch_cross_validation_summary.get("cross_validation_reason", "") or "").strip()
        batch_cross_validation_windows = dict(batch_cross_validation_summary.get("windows", {}) or {})
        batch_cross_validation_supported = batch_cross_validation_support_level in {"strong_support", "dominant_conditional"}
        batch_cross_validation_dominant = batch_cross_validation_support_level == "dominant_conditional"
        cross_validated_batch_mode = prioritized_memory_kind == "scientific_batch_reconciliation" and batch_cross_validation_supported
        hybrid_auxiliary_mode = prioritized_memory_kind == "scientific_batch_reconciliation" and not batch_cross_validation_supported
        ...
        if not accepted_candidates and prioritized_memory_kind in {"scientific_batch_reconciliation", "scientific_strong_near_miss", "scientific_reconciliation", "scientific_calibration"} and memory_policy and not hybrid_auxiliary_mode:
            accepted_candidates = [
                {
                    "variant": (
        ...
        if cross_validated_batch_mode and memory_policy:
            ...
            selected_policy["policy_variant"] = "cross_validated_scientific_batch_memory"
            ...
            selected_policy["recommended_action"] = "historical_cross_validation_supports_memory"
            selected_policy["policy_adjustment_reason"] = "historical_cross_validation_supports_memory"
            selected_policy["selection_variant"] = "cross_validated_scientific_batch_memory"
            selected_policy["selection_reason"] = "historical_cross_validation_supports_memory"
            selected_policy["selection_status"] = "POLICY_SELECTED"
            selected_policy["cross_validation_windows"] = dict(batch_cross_validation_windows)
            selected_policy["cross_validation_summary"] = dict(batch_cross_validation_summary)
            ...
            selected_policy["status_prospectivo"] = "pending_prospective_validation"
            selected_policy["reason"] = "historical_cross_validation_did_not_support_memory"
            ...
            selected_policy["previous_count_11_plus"] = batch_memory_previous_count_11_plus
            ...
            selected_policy["current_count_11_plus"] = batch_memory_current_count_11_plus
            selected_policy["recommended_action"] = "historical_cross_validation_does_not_support_memory"
            selected_policy["policy_adjustment_reason"] = "historical_cross_validation_does_not_support_memory"
            selected_policy["selection_variant"] = hybrid_selection_variant
            selected_policy["selection_reason"] = "historical_cross_validation_does_not_support_memory"
            selected_policy["selection_status"] = "POLICY_SELECTED"
        ...
        elif prioritized_memory_kind == "scientific_batch_reconciliation" and memory_policy:
            selected_policy = dict(memory_policy)
            selection_reason = "historical_cross_validation_supports_memory"
            selected_variant = "cross_validated_scientific_batch_memory"
            selected_rank = 0
            ...
            json.dumps(
                {
                    ...
                    "validation_threshold": int(selected_policy.get("validation_threshold", 0) or 0),
        ...
        if cross_validated_batch_mode:
            selection_reason = batch_cross_validation_reason or "historical_cross_validation_supports_memory"
        elif hybrid_auxiliary_mode:
            selection_reason = hybrid_selection_reason
        elif prioritized_memory_kind in {"scientific_batch_reconciliation", "scientific_strong_near_miss", "scientific_reconciliation", "scientific_calibration"} and memory_policy:
            selection_reason = prioritized_memory_row.get("policy_adjustment_reason") or prioritized_memory_row.get("recommended_action") or selection_reason
        else:
            selection_reason = (
        ...
        policy_discovery = {
            ...
            "policy_origin": "automatic_scientific_discovery" if not memory_policy else str(selected_policy.get("policy_origin", "scientific_memory")),
            "policies_tested": len(candidate_variants),
            "validation_window": int(profile_window or len(self.contests)),
            "official_history_count": history_count,
            "official_history_first_contest": int(self.contests[0].get("contest_number", 0) or 0) if self.contests else None,
            ...
            "rejected_by_guardian": rejected_by_guardian_count,
            "rejected_by_rules": sum(1 for item in scored_candidates if item["acceptance_errors"]),
            "memory_role": "strong_support" if cross_validated_batch_mode else ("auxiliary" if hybrid_auxiliary_mode else ("dominant" if memory_policy else "")),
            "dominant_memory": "conditional" if cross_validated_batch_mode else (False if hybrid_auxiliary_mode else bool(memory_policy)),
            "dominant_memory_mode": "conditional" if cross_validated_batch_mode else ("false" if hybrid_auxiliary_mode else ("true" if memory_policy else "")),
            "reason": batch_cross_validation_reason if cross_validated_batch_mode else ("historical_cross_validation_did_not_support_memory" if hybrid_auxiliary_mode else ""),
            "previous_best_hits": batch_memory_previous_best_hits if (hybrid_auxiliary_mode or cross_validated_batch_mode) else None,
            "previous_count_10": batch_memory_previous_count_10 if (hybrid_auxiliary_mode or cross_validated_batch_mode) else None,
            "previous_count_11_plus": batch_memory_previous_count_11_plus if (hybrid_auxiliary_mode or cross_validated_batch_mode) else None,
            "current_best_hits": batch_memory_current_best_hits if (hybrid_auxiliary_mode or cross_validated_batch_mode) else None,
            "current_count_10": batch_memory_current_count_10 if (hybrid_auxiliary_mode or cross_validated_batch_mode) else None,
            "current_count_11_plus": batch_memory_current_count_11_plus if (hybrid_auxiliary_mode or cross_validated_batch_mode) else None,
            "recommended_action": "historical_cross_validation_supports_memory" if cross_validated_batch_mode else ("historical_cross_validation_does_not_support_memory" if hybrid_auxiliary_mode else selection_reason),
            "policy_adjustment_reason": "historical_cross_validation_supports_memory" if cross_validated_batch_mode else ("historical_cross_validation_does_not_support_memory" if hybrid_auxiliary_mode else selection_reason),
            "cross_validation_windows": batch_cross_validation_windows if cross_validated_batch_mode else batch_cross_validation_windows if prioritized_memory_kind == "scientific_batch_reconciliation" else {},
            "cross_validation_summary": batch_cross_validation_summary if prioritized_memory_kind == "scientific_batch_reconciliation" else {},
            "cross_validation_reason": batch_cross_validation_reason if prioritized_memory_kind == "scientific_batch_reconciliation" else "",
            "based_on_memory_kind": prioritized_memory_kind or None,
            "based_on_memory_id": prioritized_memory_id or None,
            "based_on_batch_id": prioritized_batch_id or None,
            "based_on_generation_range": prioritized_generation_range or None,
