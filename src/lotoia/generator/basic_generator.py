...
def _is_valid_game(game: dict[str, object]) -> bool:
    sequence_stats = calculate_sequence_stats(game["numbers"])

    return (
        6 <= game["odd"] <= 9
        and 170 <= game["sum"] <= 240
        and 8 <= game["frame"] <= 12
        and 3 <= game["center"] <= 7
        and sequence_stats["sequence_count"] <= 3
        and sequence_stats["largest_sequence"] <= 3
    )


def _attach_scores(game: dict[str, object]) -> dict[str, object]:
...
def generate_multiple_games(count: int = 10, max_repeated: int = 9) -> list[dict[str, object]]:
    ...
    while len(games) < count and attempts < max_attempts:
        ...
        if game_key in seen_games:
            continue
        if any(_repeated_count(game, previous_game) > max_repeated for previous_game in games):
            continue
...
def generate_best_games(
    ...
    best_games = sorted(games, key=_hybrid_score_sort_key)[:count]
    return {"count": len(best_games), "games": best_games}
