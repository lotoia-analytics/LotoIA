...
def _validate_rate_limit(
    limiter_key: str, active_limiter: PublicLimiter | None = None
) -> None:
...
def _find_contest(contest_id: int, history_path: Path) -> Any:
    ...
    for draw in draws:
        if draw.contest == contest_id:
            return draw
    raise PublicContestNotFoundError(
        f"Concurso {contest_id} não encontrado na base histórica."
    )


def find_historical_matches(
    numbers: list[int], history_path: Path = DEFAULT_HISTORY_PATH
) -> dict[str, Any]:
...
def generate_public_games(
    ...
    *,
    ...
    observability_repository = (
        ObservabilityRepository(db_path) if db_path is not None else None
    )
    observability = (
        ObservabilityTracer(observability_repository)
    ...
    generation_span = (
        observability.tracer.start_span(
            "generate_public_games",
            trace_id=execution_id,
            attributes={"flow": "generation"},
        )
        if observability is not None
        else None
    )
    games_payload = generate_ranked_games(
        total_games=2, seed=seed, ml_enabled=request.ml_enabled
    )
    games_payload, score_ml_runtime = activate_score_ml_runtime(
        games_payload, enabled=request.ml_enabled
    )
    execution_time_ms = (time.time() - started_at) * 1000
    if observability is not None:
        observability.record_metric(
            execution_id,
            metrics.timing("runtime_latency_ms", execution_time_ms),
            stage="generation",
        )
        ...
        if generation_span is not None:
            ...
            observability_repository.record_span(
                execution_id, finished_generation_span, stage="generation"
            )
    ...
    if db_path is not None:
        ...
        lead = lead_repository.find_by_first_name_and_whatsapp(
            request.first_name.strip(), request.whatsapp
        )
        if lead is None:
    ...
    return {
        ...
        "metadata": {
            ...
            "profile_distribution": {
                profile: sum(
                    1 for game in games_payload if game.get("profile_type") == profile
                )
...
def check_public_contest(
    ...
    *,
    ...
    check_span = (
        observability.tracer.start_span(
            "check_public_contest", trace_id=execution_id, attributes={"flow": "check"}
        )
        if observability is not None
        else None
    )
    ...
    if observability is not None:
        ...
        if check_span is not None:
            ...
            observability_repository.record_span(
                execution_id, finished_check_span, stage="check"
            )
            observability.finish_execution(
                execution_id,
                status="ok",
