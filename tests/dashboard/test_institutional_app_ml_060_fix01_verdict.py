...
def test_classify_generation_visibility_blocks_ml_verdict() -> None:
    payload = institutional_app._classify_generation_visibility(
        generation={
            "batch_id": "batch-ml-block",
    ...
    assert payload["is_conferible"] is False
    assert payload["is_ml_verdict_blocked"] is True
    assert payload["visibility_kind"] == "ml_verdict_blocked"
    assert "Veredito ML" in payload["visibility_label"]


def test_load_persisted_groups_exposes_ml_verdict_fields(monkeypatch: pytest.MonkeyPatch) -> None:
    groups = [
        {
...
def test_persist_clean_law15_includes_ml_verdict_gate(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_conference_filters_blocked_official_groups(monkeypatch: pytest.MonkeyPatch) -> None:
    source = inspect.getsource(institutional_app._run_institutional_conference)
    assert "official_release_allowed" in source
    assert "Veredito ML" in source


def test_analytical_rows_skip_blocked_generations(monkeypatch: pytest.MonkeyPatch) -> None:
    blocked_generation: dict[str, Any] = {
    ...
    monkeypatch.setattr(
        ...
        lambda limit=25: [blocked_generation, allowed_generation],
    )
