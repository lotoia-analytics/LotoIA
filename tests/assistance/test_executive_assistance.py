...
def test_executive_assistance_reports_contextual_guidance(tmp_path: Path) -> None:
    ...
    assistance = build_executive_assistance(db_path)

    assert assistance["state"] in {"assistido", "orientado"}
    assert assistance["summary"]["presence"] in {"fully_live", "live_monitoring"}
    assert len(assistance["recommendations"]) >= 3
    assert len(assistance["explanation"]) >= 3
    assert len(assistance["guidance"]) >= 3
