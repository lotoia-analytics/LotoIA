...
def test_explainable_analytics_translate_governed_signals(tmp_path: Path) -> None:
    ...
    explainable = build_explainable_analytics(db_path)

    assert explainable["state"] in {"explicado", "observacao"}
    assert explainable["summary"]["presence"] in {"fully_live", "live_monitoring"}
    assert explainable["summary"]["historical_trend"] != ""
    assert len(explainable["explanation"]) >= 4
    assert len(explainable["narrative"]) >= 3
