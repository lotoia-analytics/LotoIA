...
from lotoia.observability import ObservabilityRepository, build_real_time_governance


def test_real_time_governance_reports_governed_state(tmp_path: Path) -> None:
    ...
    governance = build_real_time_governance(db_path)

    assert governance["summary"]["health_status"] in {"healthy", "degraded"}
    assert governance["status"] in {"governed", "watch", "alert"}
    assert governance["summary"]["blocking_count"] >= 0
    assert "health" in governance
