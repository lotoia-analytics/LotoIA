...
from lotoia.observability import ObservabilityRepository, build_live_institutional_presence


def test_live_institutional_presence_reports_compact_state(tmp_path: Path) -> None:
    ...
    presence = build_live_institutional_presence(db_path)

    assert presence["presence"] in {"fully_live", "live_monitoring"}
    assert presence["summary"]["state"] in {"live", "monitoring"}
    assert presence["summary"]["telemetry_status"] in {"live", "idle"}
