...
from lotoia.observability import ObservabilityRepository, build_operational_experience


def test_operational_experience_reports_live_state(tmp_path: Path) -> None:
    ...
    experience = build_operational_experience(db_path)

    assert experience["state"] in {"live", "monitoring"}
    assert experience["summary"]["telemetry_status"] in {"live", "idle"}
    assert experience["summary"]["memory_status"] in {"live", "standby"}
    assert len(experience["narrative"]) >= 3
