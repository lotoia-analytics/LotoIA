...
from lotoia.observability import ObservabilityRepository, build_operational_health_snapshot


def test_operational_health_snapshot_reports_status(tmp_path: Path) -> None:
    ...
    health = build_operational_health_snapshot(db_path)

    assert health["runtime_awareness"] == "connected"
    assert health["telemetry_status"] == "live"
