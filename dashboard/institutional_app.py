...
from dashboard.institutional_clean_law15_runtime import (
    ...
    render_generation_compact_header,
...
def _render_clean_law15_generation_page(snapshot: dict[str, Any]) -> None:
    snapshot = _live_institutional_snapshot(snapshot)
    blocked = _is_sovereign_generation_blocked()
    ml_active = is_adm_supervised_ml_active()
    render_generation_compact_header(ml_active=ml_active, generation_active=not blocked)
    if blocked:
        st.error(
    ...
    requested_count = render_generation_operation_block(ml_active=ml_active)
    ...
    if result:
        render_generation_result_summary(result, diagnostics=diagnostics)
        games = list(
            result.get("display_games")
            or _expand_generation_games_for_format(result.get("games") or [], ADM_RUNTIME_ACTIVE_CARD_FORMAT)
        )
        render_generation_games_table(
            games,
