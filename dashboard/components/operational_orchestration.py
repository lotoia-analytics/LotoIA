    ...
    with st.container(border=True):
        ...
        with st.expander("Detalhes operacionais", expanded=False):
            st.info(
                ...
                f" | comparacao: {decision_context.get('comparison', '-')}"
            )
            ...
            if storytelling:
                ...
                st.info("Narrativa operacional em consolidacao.")

            events_frame = pd.DataFrame(events)
            if events_frame.empty:
                st.info("Sem eventos institucionais consolidados.")
            else:
            ...
            if coordination_frame.empty:
                st.info("Coordenacao viva em consolidacao.")
            else:
            ...
            signal_cols[1].metric("Padrao", signal_engine.get("pattern", "observacao governada"))
            signal_cols[2].metric("Mudancas", signal_engine.get("persistent_changes", 0))
            signal_cols[3].metric("Recorrencia", signal_engine.get("recurring_statuses", 0))
            ...
            st.caption(
                ...
                f" | coordenacao: {institutional_presence.get('coordination_depth', 0)}"
            )
