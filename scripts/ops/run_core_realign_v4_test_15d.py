#!/usr/bin/env python3
"""Piloto V4: STRUCT_LEI15_CORE_V4_PATTERN_PROTECTED_15D_001 (padrões V1, sem cartões exatos).

Uso:
  python scripts/ops/run_core_realign_v4_test_15d.py --generations 1
  python scripts/ops/run_core_realign_v4_test_15d.py --validate-only
  python scripts/ops/run_core_realign_v4_test_15d.py --pilot-hits-check
  python scripts/ops/run_core_realign_v4_test_15d.py --target-total 20
"""
...
LOT_GATE_BEST_HIT = 13
LOT_GATE_AVG_HIT = 11.5
LOT_GATE_RUNS_13_PLUS = 20
...
def _is_v4_applied(game: dict) -> bool:
    return bool(
        game.get("core_realignment_v4_applied") is True
        or (game.get("realignment_metadata") or {}).get("v4_applied") is True
    )


def _expected_faixa_a_per_gp() -> int:
    return max(1, int(round(GAMES_COUNT * PATTERN_RATIO)))
...
def _generate_games_progressive(*, seed: int, pool_steps: list[int]) -> tuple[list[dict], int]:
    ...
    for pool_size in pool_steps:
        ...
        faixa_a = sum(
            1 for g in games if (g.get("realignment_metadata") or {}).get("protected_v1_pattern") is True
        )
        _log(
            f"  resultado: {len(games)} jogos | v4={sum(1 for g in games if _is_v4_applied(g))} "
            f"faixa_a={faixa_a} target_a={_expected_faixa_a_per_gp()}"
        )
...
def _persist(*, games: list[dict], seed: int, contest_number: int, created_by: str) -> dict:
    ...
    ctx = {
        ...
        **meta,
        ...
        "generation_mode": "LEI15_CORE_REALIGNMENT_V4_PATTERN_PROTECTED_SHADOW_TEST_HEADLESS",
        "core_realignment_v4_applied_count": sum(1 for g in games if _is_v4_applied(g)),
        "protected_v1_pattern_count": faixa_a,
        "core_realignment_v4_applied": any(_is_v4_applied(g) for g in games),
        "pattern_protected_ratio": PATTERN_RATIO,
    }
    snap = _persist_generation_snapshot(
        games=games,
        seed=int(seed),
        target_contest=int(contest_number),
        batch_id=f"core-v4-pattern-15d-c{contest_number}-{uuid.uuid4().hex[:8]}",
...
def _validate_db() -> dict:
    ...
    expected_a = _expected_faixa_a_per_gp()
    url = resolve_database_url()[0]
    with psycopg.connect(url) as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                SELECT COUNT(DISTINCT ge.id), COUNT(gg.id),
                       COUNT(gg.id) FILTER (
                         WHERE gg.context_json::text ILIKE '%%"v4_applied": true%%'
                            OR gg.context_json::text ILIKE '%%"core_realignment_v4_applied": true%%'
                       ),
                       COUNT(gg.id) FILTER (
                         WHERE gg.context_json::text ILIKE '%%"protected_v1_pattern": true%%'
                       ),
                       COUNT(gg.id) FILTER (
            ...
            total, games, v4, faixa_a, v3_1, v3_legacy = cur.fetchone()

    batch = {
        "label": BATCH_LABEL,
        "total": int(total or 0),
        "games": int(games or 0),
        "games_v4_applied": int(v4 or 0),
        "games_faixa_a": int(faixa_a or 0),
        "games_v3_1": int(v3_1 or 0),
        "games_v3_legacy": int(v3_legacy or 0),
        "expected_faixa_a_per_gp": expected_a,
    }
    ok = bool(
        batch["total"] >= 1
        and batch["games"] >= GAMES_COUNT
        and batch["games_v4_applied"] == batch["games"]
        and batch["games_faixa_a"] == batch["total"] * expected_a
        and batch["games_v3_1"] == 0
...
def _print_validation(val: dict) -> None:
    ...
    _log(
        ...
        f"games={batch.get('games', 0):>4} v4={batch.get('games_v4_applied', 0):>4} "
        f"faixa_a={batch.get('games_faixa_a', 0):>4} v3_1={batch.get('games_v3_1', 0):>3} "
        f"v3_legacy={batch.get('games_v3_legacy', 0):>3} [{status}]"
    )


def _pilot_metadata_checks(batch: dict) -> list[tuple[str, bool]]:
    expected_a = batch.get("expected_faixa_a_per_gp", _expected_faixa_a_per_gp())
    per_ge_games = GAMES_COUNT
    per_ge_faixa_a = expected_a
    return [
        (
            f"v4_applied = {per_ge_games}/{per_ge_games}",
            batch.get("games_v4_applied") == batch.get("games") == per_ge_games,
        ),
        (
            f"pattern_protected = {per_ge_faixa_a}/{per_ge_games}",
            batch.get("games_faixa_a") == per_ge_faixa_a and batch.get("total") == 1,
        ),
        ("v3_1_applied = 0", batch.get("games_v3_1") == 0),
        ("v3_legacy = 0", batch.get("games_v3_legacy") == 0),
        ("jogos invalidos = 0", batch.get("games") == per_ge_games),
    ]
...
def _pilot_hits_check() -> int:
    ...
    _log("=== PILOT CHECK V4 (metadata + hits 3705-3711) ===")
    val = _validate_db()
    _print_validation(val)
    meta_checks = _pilot_metadata_checks(val.get("batch") or {})
    meta_passed = sum(1 for _, ok in meta_checks if ok)
    for name, ok in meta_checks:
        _log(f"  [{'OK' if ok else 'PENDENTE'}] {name}")
    _log(f"[PILOT V4] metadata {meta_passed}/{len(meta_checks)}")
    if not val.get("validation_ok"):
        _log("[PILOT] metadata/validacao DB reprovada — abortando hits")
        return 1

    rc = subprocess.call(
        [sys.executable, str(ROOT / "scripts" / "ops" / "reconcile_core_realign_v4_baseline_contests.py")],
        cwd=str(ROOT),
    )
    if rc != 0:
    ...
    hit_checks = [
        ...
        (f"runs 13+ >= {PILOT_GATE_RUNS_13_PLUS}", runs_13_plus >= PILOT_GATE_RUNS_13_PLUS),
    ]
    ...
    for name, ok in hit_checks:
        _log(f"  [{'OK' if ok else 'PENDENTE'}] {name}")
        hit_passed += int(ok)
    _log(f"[PILOT V4] hits {hit_passed}/{len(hit_checks)}")
    total_checks = len(meta_checks) + len(hit_checks)
    total_passed = meta_passed + hit_passed
    _log(f"[PILOT V4] total {total_passed}/{total_checks}")
...
def main() -> int:
    ...
    parser.add_argument("--created-by", default="ops/run_core_realign_v4_test_15d")
    ...
    if os.environ.get("LOTOIA_LEI15_CORE_REALIGNMENT_V4", "off") != "shadow_test":
        raise RuntimeError("V4 exige LOTOIA_LEI15_CORE_REALIGNMENT_V4=shadow_test")
